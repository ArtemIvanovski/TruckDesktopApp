...
class TabSystem:
    ...
    def _add_box(self):
        print("Добавить коробку")

    def _delete_box(self):
        print("Удалить коробку")

    def _copy_box(self):
        print("Копировать коробку")

    def _auto_place(self):
        print("Автоматическое размещение")

    def _clear_boxes(self):
        print("Очистить коробки")
...
class TrailerUI:
    ...
    def _build_right_panel(self):
        ...
        # Поля ввода размеров
        self.input_len = DirectEntry(text=str(self.app.truck_width), scale=0.045, pos=(1.15, 0, 0.25),
                                     initialText=str(self.app.truck_width), numLines=1, focus=0)
        self.input_wid = DirectEntry(text=str(self.app.truck_depth), scale=0.045, pos=(1.45, 0, 0.25),
                                     initialText=str(self.app.truck_depth), numLines=1, focus=0)
        self.input_hei = DirectEntry(text=str(self.app.truck_height), scale=0.045, pos=(1.75, 0, 0.25),
                                     initialText=str(self.app.truck_height), numLines=1, focus=0)

        self.button_apply_dims = DirectButton(text=t(LANG, 'btn_ok'), scale=0.045, pos=(2.0, 0, 0.25),
                                              command=self._apply_dims)
